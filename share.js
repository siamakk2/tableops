(function(){
  ...
  var ICON={
    ...
    cl:'<svg viewBox="0 0 24 24"><path d="M10 13a5 5 0 0 0 7.07 0l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71" fill="none" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><path d="M14 11a5 5 0 0 0-7.07 0l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71" fill="none" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>'
  };
  var css='.shz{display:flex;align-items:center;justify-content:center;flex-wrap:wrap;gap:12px;padding:24px 16px;border-top:1px solid rgba(127,127,127,.18);font-family:inherit;}'
   ...
   +'.shz button{width:42px;height:42px;border-radius:50%;display:inline-flex;align-items:center;justify-content:center;border:none;cursor:pointer;padding:0;transition:transform .12s,box-shadow .12s;-webkit-tap-highlight-color:transparent;}'
   +'.shz button:hover{transform:translateY(-2px);box-shadow:0 6px 16px rgba(0,0,0,.28);}'
   +'.shz svg{width:20px;height:20px;fill:#fff;}'
   +'.shz .fb{background:#1877F2;}.shz .li{background:#0A66C2;}.shz .cl{background:#475569;}'
   +'.shz .ig{background:radial-gradient(circle at 30% 107%,#fdf497 0%,#fd5949 45%,#d6249f 70%,#285AEB 100%);}'
   +'.shz .tk{background:#010101;}'
   +'.shz-t{position:fixed;left:50%;bottom:24px;transform:translateX(-50%);background:#111;color:#fff;padding:11px 18px;border-radius:10px;font-size:13.5px;z-index:2147483600;opacity:0;pointer-events:none;transition:opacity .2s;font-family:inherit;max-width:88vw;text-align:center;}'
   +'.shz-t.on{opacity:1;}';
  ...
  bar.innerHTML='<span class="shz-l">Share this page</span>'
   +'<button class="fb" type="button" data-net="facebook" aria-label="Share on Facebook">'+ICON.fb+'</button>'
   +'<button class="li" type="button" data-net="linkedin" aria-label="Share on LinkedIn">'+ICON.li+'</button>'
   +'<button class="ig" type="button" data-net="instagram" aria-label="Share to Instagram">'+ICON.ig+'</button>'
   +'<button class="tk" type="button" data-net="tiktok" aria-label="Share to TikTok">'+ICON.tk+'</button>'
   +'<button class="cl" type="button" data-net="copy" aria-label="Copy link">'+ICON.cl+'</button>';
  function place(){
  ...
  var WEB={ facebook:'https://www.facebook.com/sharer/sharer.php?u='+U, linkedin:'https://www.linkedin.com/sharing/share-offsite/?url='+U };
  var PRETTY={ facebook:'Facebook', linkedin:'LinkedIn', instagram:'Instagram', tiktok:'TikTok' };
  function toast(msg){ var t=document.createElement('div'); t.className='shz-t'; t.textContent=msg; document.body.appendChild(t); requestAnimationFrame(function(){t.classList.add('on');}); setTimeout(function(){t.classList.remove('on'); setTimeout(function(){if(t.parentNode)t.remove();},300);},2600); }
  function copyLink(then){ if(navigator.clipboard&&navigator.clipboard.writeText){ navigator.clipboard.writeText(cu).then(then,then); } else { var ta=document.createElement('textarea'); ta.value=cu; ta.style.position='fixed'; ta.style.opacity='0'; document.body.appendChild(ta); ta.focus(); ta.select(); try{document.execCommand('copy');}catch(e){} ta.remove(); then(); } }
  function popup(url){ var w=window.open(url,'shzshare','width=600,height=560,menubar=no,toolbar=no,location=no,resizable=yes,scrollbars=yes'); if(w){ try{w.focus();}catch(e){} return true; } return false; }

  bar.addEventListener('click',function(e){
    var b=e.target.closest('button[data-net]'); if(!b)return;
    var net=b.getAttribute('data-net');
    if(net==='copy'){ copyLink(function(){ toast('Link copied!'); }); return; }
    // 1) phone / modern browser: open the real native share menu (this is what actually opens the apps)
    if(navigator.share){ navigator.share({ title:document.title, url:cu }).catch(function(){}); return; }
    // 2) desktop: Facebook & LinkedIn have working web dialogs
    if(WEB[net] && popup(WEB[net])) return;
    // 3) Instagram / TikTok (no web share) or popup blocked: copy + tell them what to do
    copyLink(function(){ toast('Link copied — open '+(PRETTY[net]||'the app')+' and paste it'); });
  });
})();
